...
names(survey_data)[8] = "upload_menus"

###Create a new table for Chart###
survey_data1 <- survey_data%>%
  group_by(ESC_Region) %>% 
  count(ESC_Region, upload_menus) %>%
  mutate(pct = n/sum(n))

###Stacked Bar Chart
p <- ggplot(survey_data1, aes(ESC_Region, n, fill = upload_menus)) +
  ...
  labs(title = "Survey for Utilization of uploading menus to website or phone application",
