...
names(survey_data)[6] = "utilize_electronic_food_prod_records"

###Create a new table for Chart###
survey_data1 <- survey_data%>%
                group_by(ESC_Region) %>% 
                count(ESC_Region, utilize_electronic_food_prod_records) %>%
                mutate(pct = n/sum(n))

###Stacked Bar Chart
p <- ggplot(survey_data1, aes(ESC_Region, n, fill = utilize_electronic_food_prod_records)) +
     ...
     scale_fill_discrete(name = "Utilize\nfood\nproduction\nrecords") +
     labs(title = "Survey for Utilization of Food Production Records",
